...
def discover_files(input_dir):
    ...
    """
    hist_pat = re.compile(r"(Hist\w+)_ctFIRE_(.+)_t(\d+)_s(\d+)\.csv$", re.IGNORECASE)
    ol_pat   = re.compile(r"OL_ctFIRE_(.+)_t(\d+)_s(\d+)\.tif$",         re.IGNORECASE)

    hist_files = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
    ol_files = defaultdict(lambda: defaultdict(dict))
    ...
    for fname in os.listdir(input_dir):
        ...
        if m:
            htype, stack, timepoint, slc = m.group(1), m.group(2), int(m.group(3)), int(m.group(4))
            hist_files[stack][timepoint][htype][slc] = fpath
            continue

        m = ol_pat.match(fname)
        if m:
            stack, timepoint, slc = m.group(1), int(m.group(2)), int(m.group(3))
...
def load_all_measurements(hist_files):
    ...
    for stack, timepoints in hist_files.items():
        for timepoint, htypes in timepoints.items():
            for htype, slices in htypes.items():
                for slc, path in slices.items():
                    vals = load_measurements(path)
                    if len(vals) > 0:
                        data[stack][timepoint][htype][slc] = vals
    return data
...
def compute_stats(data, z_step):
    ...
    """
    rows = []
    for stack, timepoints in data.items():
        for tpoint, htypes in timepoints.items():
            for htype, slices in htypes.items():
                for slc, vals in sorted(slices.items()):
                    rows.append({
                        "stack":     f"{stack}_t{tpoint}_s{slc}",
                        "timepoint": tpoint,
                        "hist_type": htype,
                        "slice":     slc,
                        "z_depth":   slc * z_step,
...
def build_overlay_zstacks(ol_files, stacks, output_dir):
    ...
    """
    for stack in stacks:
        tpoints = ol_files.get(stack)

        for tpoint, slices in tpoints.items():
            frames = []
        ...
        out_path  = os.path.join(output_dir, f"OL_zstack_{stack}_t{tpoint}.tif")
